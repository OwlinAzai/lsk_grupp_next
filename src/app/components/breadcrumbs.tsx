...
export default function Breadcrumbs() {
  ...
  const BreadcrumbsArray = useMemo(() => pathname.split("/").filter(Boolean), [pathname]);

  const [productName, setProductName] = useState<string | null>(null); 
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  const productNameRef = useRef<string | null>(null); // Кешируем название продукта
  const lastItem = BreadcrumbsArray[BreadcrumbsArray.length - 1]; // ID продукта, если есть
  ...
  // Используем useMemo, чтобы избежать лишних запросов при одном и том же ID
  useEffect(() => {
    const fetchProductName = async () => {
      if (!lastItem || isNaN(Number(lastItem))) return; // Проверяем, что lastItem — это число (ID продукта)

      if (productNameRef.current === lastItem) return; // Если данные о продукте уже загружены для данного ID, не загружать заново
      ...
      try {
        ...
        productNameRef.current = productData.product_name; // Сохраняем в кеше
      } catch (error) {
