  ...
  return (
    <div className="pt-6 pb-5 mx-auto sm:ml-4 pl-4 pr-4 sm:mr-4 lg:ml-32 lg:mr-32 mt-4 mb-4 shadow-xl rounded-lg px-4 bg-white">
      <ul className="breadcrumbs flex items-center text-base md:text-xl">
        ...
        <span className="breadcrumb-separator align-middle text-lg md:text-xl pl-2 md:pl-1 pr-2">
          {"▶"}
        </span>
        ...
        {BreadcrumbsArray.map((item, index) => {
          ...
          return (
            <li key={index} className="mr relative">
              <Link
                className="hover:text-orange-400 font-regular text-base md:text-xl"
                href={"/" + BreadcrumbsArray.slice(0, index + 1).join("/")}
              >
                {displayName}
              </Link>
              {index < BreadcrumbsArray.length - 1 && (
                <span className="breadcrumb-separator pl-3 md:pl-4 text-lg md:text-xl">
                  {"▶"}
                </span>
              )}
